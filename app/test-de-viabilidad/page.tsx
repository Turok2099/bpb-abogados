...
import { submitViabilidad } from "../actions/leads";
...
export default function TestDeViabilidad() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await submitViabilidad(null, formData);
    
    if (result.success) {
      toast.success("Documentación enviada exitosamente. Nuestro equipo se contactará pronto.", {
        style: {
              ...
              <form className="relative z-10 space-y-8" onSubmit={handleSubmit}>
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="space-y-2">
                    ...
                    <input name="nombre" className="w-full bg-transparent border-b-2 border-white/20 focus:border-secondary transition-colors py-3 text-white outline-none" type="text" required disabled={isPending} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-label uppercase tracking-widest text-white/70">Cargo / Empresa</label>
                    <input name="cargo" className="w-full bg-transparent border-b-2 border-white/20 focus:border-secondary transition-colors py-3 text-white outline-none" type="text" required disabled={isPending} />
                  </div>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="space-y-2">
                    ...
                    <input name="email" className="w-full bg-transparent border-b-2 border-white/20 focus:border-secondary transition-colors py-3 text-white outline-none" type="email" required disabled={isPending} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-label uppercase tracking-widest text-white/70">Teléfono / WhatsApp</label>
                    <input name="telefono" className="w-full bg-transparent border-b-2 border-white/20 focus:border-secondary transition-colors py-3 text-white outline-none" type="tel" required disabled={isPending} />
                  </div>
                </div>
                ...
                <div className="space-y-4 pt-6">
                  ...
                  <label className="border-2 border-dashed border-white/20 hover:border-secondary/50 transition-colors rounded-xl p-10 flex flex-col items-center justify-center text-center cursor-pointer group bg-surface-container-low/50 block">
                    ...
                    <p className="text-on-surface-variant text-sm mb-4">Tamaño máximo por archivo: 20MB</p>
                    <input name="archivos" type="file" multiple className="hidden" disabled={isPending} onChange={(e) => {
                      if (e.target.files && e.target.files.length > 0) {
                        setFileName(`${e.target.files.length} archivo(s) seleccionado(s)`);
                      } else {
                        setFileName("Ningún archivo seleccionado");
                      }
                    }} />
                    <p className="text-secondary text-sm font-semibold">{fileName}</p>
                  </label>
                </div>

                <div className="pt-8">
                  <button type="submit" className="w-full bg-secondary text-surface-container font-bold text-sm tracking-[0.2em] uppercase py-5 rounded-sm hover:bg-white transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed" disabled={isPending}>
                    {isPending ? "ENVIANDO DOCUMENTACIÓN..." : "ENVIAR DOCUMENTACIÓN"}
                  </button>
                </div>
              </form>
            </div>
